...
### Імпортує бібліотеку SimpleLightbox та файл стилів simple-lightbox.min.css. Це робиться за допомогою import.

Створює екземпляр SimpleLightbox, який ініціалізується за допомогою new
SimpleLightbox(). Елементи галереї вказуються як параметр, у вашому випадку це
всі посилання a в елементі з класом .gallery. Опції бібліотеки також можуть бути
передані. У цьому випадку використовується captionsData: "alt" для використання
атрибуту alt як підпису до зображень і captionDelay: 250 для встановлення
затримки перед відображенням підпису.

Створює масив images, який містить дані про зображення: посилання на маленьке
зображення (preview), посилання на велике зображення (original) та опис.

Знаходить елемент DOM з класом .gallery.

Вставляє HTML-рядок для кожного зображення в галерею за допомогою методу
insertAdjacentHTML. Це робиться з використанням методу map, який створює
HTML-рядок для кожного об'єкта image у масиві images. Потім метод join("")
об'єднує ці рядки в один.

Виводить gallery у консоль, щоб перевірити, чи правильно був знайдений
відповідний DOM-елемент.

Ініціалізує SimpleLightbox після того, як усі зображення були додані до DOM за
допомогою DOMContentLoaded event listener.
